function [X, Var_noise, outinfo] = corrCEL0_VarN_GPU(Y, Gl, replambdah, Gh0, SmSm, Lip, q, gamma, opts)
...
while (it <= opts.maxit) && ~conv
    ...
    Xp = X;
    sigbp = sigb;
    [X, sigb, out_x] = wl1fista(Y, GlkGl, GlGl, replambdah, Gh0, SmSm, X, sigb, opts.b, w, Lip, opts.opts_fista);
    ii = abs(X(:)) < sq2g_div_nG(:);
    ...
    if opts.computef
        [outinfo.fval(it), err, regval] = costfncelo(Y, X, sigb, opts.b, GlkGl, gamma, sq2g_div_nG, normsG2, ii);
    end
...
function [fval, err, reg] = costfncelo(Y, X0, sigb, b, GlkGl, gamma, sq2g_div_nG, normsG2, ii)
...
function [X, sigb, out_x] = wl1fista(Y, GlkGl, GlGl, replambdah, Gh0, SmSm, X, sigb, b, w, Lip, opts_fista)
