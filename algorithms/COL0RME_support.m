function sol_col0rme_supp = COL0RME_support(Ynoisy,opts)
    ...
    % FISTA options
    col0rme_supp.opts.opts_fista.maxit = 4e2;
...
%     col0rme_supp.opts.X0 = opts.init_X;
    ...
    if ~isfile([opts.folderSave 'COL0RME_supp_tubulin_' opts.bg '_SNR' num2str(opts.SNR) '_K' num2str(opts.K) '_N' num2str(N) patch '_' opts.reg '.mat'])

        % Computing covariance matrix
        Ry = cov(munfold(Ynoisy,3,[2 1]));
        % Ry = cov(munfold(Ynoisy/max(Ynoisy(:)),3,[2 1]));
        % A = set2zero(size(Ynoisy,1), 11);
        % Ry(A == 0) =0; 
        norm_factor = max(Ry(:));
        Ry = Ry/norm_factor;
        ...
        % col0rme_supp.gamma0 = (col0rme_supp.gamma0(:).^2)./(sqrt(2*col0rme_supp.normsG(:).^2).^2); check
        col0rme_supp.gamma0_cel0 = col0rme_supp.gamma0(:) ./ sqrt(2*col0rme_supp.normsG(:).^2);
        ...
        switch opts.reg
            ...
            case 'CEL0_l1'
                if opts.GPU
                    [sol_col0rme_supp.Rx, sol_col0rme_supp.VarNoise, sol_col0rme_supp.outinfo] = corrCEL0_VarN_L1_GPU(Ry(:), Gl, replambdah,...
                                    Gh0, SmSm, Lip, opts.srf, col0rme_supp.gamma * col0rme_supp.gamma0_cel0 * ones(L^2,1), col0rme_supp.opts);
                    sol_col0rme_supp.Rx = gather(sol_col0rme_supp.Rx);
                else
            ...
            case 'L1'
                if opts.GPU
                    if opts.VarEst
                        ...
                        [sol_col0rme_supp.Rx, sol_col0rme_supp.VarNoise] = corrL1_noVarN_GPU(Ry(:), Gl, Lip, opts.srf,...
                                        col0rme_supp.gamma * col0rme_supp.gamma0_l1 , col0rme_supp.opts);
                        sol_col0rme_supp.Rx = gather(sol_col0rme_supp.Rx); 
            ...
            case 'TV'
                if opts.GPU
                    [sol_col0rme_supp.Rx, sol_col0rme_supp.it, sol_col0rme_supp.time, sol_col0rme_supp.crit, sol_col0rme_supp.VarNoise] ...
                        = corrTV_VarN_GPU(Ry(:), Gl, Lip, col0rme_supp.gamma * col0rme_supp.gamma0);
                    sol_col0rme_supp.Rx = gather(sol_col0rme_supp.Rx); 
                else
                    error('Only GPU implementation. Try CEL0_0 regularizer for CPU implementation');
                end
        ...
        % Storing results
        save([opts.folderSave 'COL0RME_supp_tubulin_' opts.bg '_SNR' num2str(opts.SNR) '_K' num2str(opts.K) '_N' num2str(N) patch '_' opts.reg ], 'col0rme_supp', 'sol_col0rme_supp');
    else 
        load([opts.folderSave 'COL0RME_supp_tubulin_' opts.bg '_SNR' num2str(opts.SNR) '_K' num2str(opts.K) '_N' num2str(N) patch '_' opts.reg '.mat'],'sol_col0rme_supp')
    end
